...
openshift_post_install:
	...
	for i in $(cat ~/inventory.cfg | grep openshift.local | awk {'print $1'} | grep -v "#" | sort | uniq -u | grep -v master); do cat ~/postinstall-node.sh | ssh -A ec2-user@$i ; done
	for i in $(cat ~/inventory.cfg | grep openshift.local | awk {'print $1'} | grep -v "#" | sort | uniq -u | grep master); do cat ~/postinstall-master.sh | ssh -A ec2-user@$i ; done